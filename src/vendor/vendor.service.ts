...
export class VendorService {
  ...
  async create(createVendorDto: any, tenantId: string): Promise<Vendor> {
    const vendorData = {
      ...createVendorDto,
      tenantId,
      rating: 0,
    ...
    const vendor = this.vendorRepository.create(vendorData as Partial<Vendor>);
    const savedVendor = await this.vendorRepository.save(vendor);
    return Array.isArray(savedVendor) ? savedVendor[0] : savedVendor;
  }

  async update(id: string, updateVendorDto: any, tenantId: string): Promise<Vendor> {
    const vendor = await this.findOne(id, tenantId);
    Object.assign(vendor, updateVendorDto);
    return this.vendorRepository.save(vendor);
  }

  async toggleBlacklist(id: string, tenantId: string, reason?: string): Promise<Vendor> {
    const vendor = await this.findOne(id, tenantId);
  ...
  async seedVendors(tenantId: string): Promise<Vendor[]> {
  ...
  async calculateVendorPerformance(vendorId: string, tenantId: string): Promise<VendorPerformanceMetrics> {
    const transactions = await this.transactionRepository.find({
      where: { vendorId, tenantId, status: TransactionStatus.COMPLETED },
      order: { createdAt: 'DESC' },
    });
    ...
    const inspectionFailureCount = rejectedTransactions.filter(tx =>
      tx.inspectionData?.rejectionReason && tx.currentLevel >= OperationalLevel.L4_MATERIAL_INSPECTION
    ).length;
  ...
  async updateVendorPerformanceMetrics(vendorId: string, tenantId: string): Promise<void> {
  ...
  async calculateVendorRiskScoring(vendorId: string, tenantId: string): Promise<VendorRiskScoring> {
  ...
  async getVendorTrendAnalysis(vendorId: string, tenantId: string, period: 'DAILY' | 'WEEKLY' | 'MONTHLY' = 'MONTHLY', periodCount: number = 12): Promise<VendorTrendAnalysis> {
    const vendor = await this.vendorRepository.findOne({ where: { id: vendorId, tenantId } });
    if (!vendor) {
      throw new NotFoundException(`Vendor with ID ${vendorId} not found`);
    }
  ...
  async getVendorRealTimeMetrics(tenantId: string): Promise<{ totalVendors: number; activeVendors: number; highRiskVendors: number; averageRejectionRate: number }> {
  ...
  async getVendorPerformanceRanking(tenantId: string, limit: number = 10): Promise<{ topPerformers: VendorRiskScoring[]; worstPerformers: VendorRiskScoring[] }> {
